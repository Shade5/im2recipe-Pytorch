...
class im2recipe(nn.Module):
    def __init__(self):
        ...
        if opts.preModel == 'resNet50':
            ...
            self.visual_embedding = nn.Sequential(
                nn.Linear(opts.irnnDim*2 + opts.imfeatDim, opts.embDim),
                nn.Tanh(),
            )
            
            self.recipe_embedding = nn.Sequential(
                nn.Linear(opts.srnnDim, opts.embDim, opts.embDim),
                nn.Tanh(),
            )
    ...
    def forward(self, x, y1, y2, z1, z2):
        ...
        recipe_emb = self.recipe_embedding(self.stRNN_(y1, y2))
        recipe_emb = norm(recipe_emb)

        # visual embedding
        visual_emb = self.visionMLP(x)
        visual_emb = visual_emb.view(visual_emb.size(0), -1)
        visual_emb = self.table([visual_emb, self.ingRNN_(z1, z2)], 1)
